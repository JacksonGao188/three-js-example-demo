...
const VisualSortingAlgorithm: FC = () => {
  ...
  const onStartSort = async () => {
    ...
    if (curSortName === "quickSort")
      await quickSort(randomArr, 0, randomArr.length - 1);
  ...
  const insertionSort = async (arr: RandomArrType) => {
    ...
    for (let i = 1; i < arrLen; i++) {
      ...
      if (preIndex + 1 !== i) {
        ...
        await sleep();
        curMesh.position.y = Group.getObjectByName(
          arr[preIndex + 1].name
        )?.position.y;
      }
      curMesh.position.z = curMesh.position.z * -1;
  ...
  const exchangeMeshPositionByName = (
    leftMeshName: string,
    rightMeshName: string
  ): Promise<string> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        [leftMesh.position.y, rightMesh.position.y] = [
          rightMesh.position.y,
          leftMesh.position.y,
        ];
        resolve("success");
      }, 1000);
    ...
    <>
      <div className={styles.sortSpace}>
        <Space align="center">
          ...
          />
          <Button onClick={onCreateRandomArr} disabled={isSorting}>
            生成随机数组
          </Button>
          <Button onClick={onStartSort} disabled={isSorting}>
            开始排序
          </Button>
        </Space>
      </div>
